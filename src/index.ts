...
export const highlightText = (text: string, colors: ColorMap): string => {
	...
	const allDefinedRuleActions = new Set<string>();
	const definitionLineToActionMap = new Map<string, string>();
	const labeledRules = new Map<string, string>();
	...
	const rulesWithExternalReferences = new Set<string>();
		...
		for (const definedAction of allDefinedRuleActions) {
			...
			lineHtml = lineHtml.replace(generalReferencePattern, (match, p1) => {
				...
				return createPlaceholder(
					`<span class="${colors.reference}">${match}</span>`,
				);
			});
		}
	...
	html = html.replace(
		...
		(match, labelName, _predicate) => {
			if (labeledRules.has(labelName)) {
				return createPlaceholder(
					`<span class="${colors.labelReference}">${match}</span>`,
				);
			}
			return match;
		},
	...
	html = html.replace(
		/\b(sqrt|min|max|clamp|floor|ceil|ceiling|round|absolute|abs)(?=\s*\()/g,
		(match) => {
			return createPlaceholder(
				`<span class="${colors.function}">${match}</span>`,
			);
